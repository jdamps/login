...
/*gdy user nie wypelni wszystkich pol*/
	if (empty($login) || empty($haslo1) || empty($haslo2)) {
		header ("Location: ../adminemployadd.php?error=emptyfields");
		exit();
	}	
...
/*sprawdzanie powtorzonego hasla*/
		...
		header ("Location: ../adminemployadd.php?error=passwordcheck");
	}
...
/*sprawdzanie czy taki user juz istnieje*/
		...
		if (!mysqli_stmt_prepare($stmt, $sql)) {
			header ("Location: ../adminemployadd.php?error=sqlerrorr");
			exit();
		} 
			...
			if ($resultCheck > 0) {
				header ("Location: ../adminemployadd.php?error=usertaken");
				exit();
			}
			else {
				...
				$stmt = mysqli_stmt_init($con);
				if (!mysqli_stmt_prepare($stmt, $sql)) {
					header ("Location: ../adminemployadd.php?error=sqlerror");
					exit();
				} 
				else  {
					...
					header ("Location: ../adminemployadd.php?signup=success");
					exit();
	...
	mysqli_close($con);
		...
		header ("Location: ../adminemployadd.php");
		exit();
